/**
 * If not using bull, remove bull from dependencies
 * and replace this content with content from createClient.redis.noBull.ts
 */
...
const createClient: Required<QueueOptions, 'createClient'>['createClient'] = (type, redisOpts?) => {
  switch (type) {
    case 'client':
      if (!client) {
